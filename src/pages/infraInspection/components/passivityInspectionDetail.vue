  ...
  <div class="responsive-row">
    <el-row :gutter="10" class="mb-20">
      <el-col :xs="24" :sm="24" :md="8" style="display: flex; flex-direction: column;">
        <BaseCard title="데이터베이스 수동점검(2024)">
    ...
    <el-row :gutter="10" class="responsive-row-inner">
      <el-col :xs="24" :sm="12" :md="8">
        <el-row  class="responsive-row-inner data-list">
          <el-col :xs="24" :sm="12" :md="12" style="padding-right: 5px;">
            <div class="mb-20">
              <h4>점검계획</h4>
            </div>
            <BaseButton @click="openInspectInsertModal" class="white-button w-full">점검계획 등록&nbsp;<el-icon><Plus/></el-icon></BaseButton>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" style="padding-left: 5px;">
            <div class="mb-20">
              <h4>점검대상</h4>
            </div>
          </el-col>
    ...
    <passivityInspeceionModal
      :visible="InspectModalVisible"
      :data="InspectData"
      @close="closeInspectModal"
    />

    <passivityInspectInsertModal
      :visible="InspectplantModalVisible"
    ...
    <passivityInspectResultOpinionDialog
...
<script setup>
import { ref } from "vue";
import passivityInspeceionModal from '../dialogs/passivityInspeceionModal.vue';
import passivityInspectInsertModal from '../dialogs/passivityInspectInsertModal.vue';
import passivityInspectResultNoticeDialog from '../dialogs/passivityInspectResultNoticeDialog.vue';
import passivityInspectResultOpinionDialog from '../dialogs/passivityInspectResultOpinionDialog.vue';
...
// 조치 증적 등록 팝업 관련
const InspectplantModalVisible = ref(false);
const openInspectInsertModal = () => {
  InspectplantModalVisible.value = true;
}
const closeInspectInsertModal = () => {
  InspectplantModalVisible.value = false;
}
...
const chartData = ref([
  { name: "점검 미등록", value: 55, color: "#333333" },
  { name: "점검등록", value: 45, color: "#F56C6C" }
]);
// 데이터 리스트
const tableData = [{
...
const dumiData = [
  {code: 'SRV-012', category: '.netrc 파일 내 중요 정보 노출', dangerousLevel: '상', firstResult: '양호', lastResult: '취약', vulnerable: 'ckeck',  exception: '', state: '양호', record: 'check'},
  {code: 'SRV-012', category: '.netrc 파일 내 중요 정보 노출', dangerousLevel: '상', firstResult: '양호', lastResult: '취약', vulnerable: 'ckeck',  exception: '', state: '양호', record: 'check'},
  {code: 'SRV-012', category: '.netrc 파일 내 중요 정보 노출', dangerousLevel: '상', firstResult: '양호', lastResult: '취약', vulnerable: 'ckeck',  exception: '', state: '양호', record: 'check'},
  {code: 'SRV-012', category: '.netrc 파일 내 중요 정보 노출', dangerousLevel: '상', firstResult: '양호', lastResult: '취약', vulnerable: 'ckeck',  exception: '', state: '양호', record: 'check'},
  {code: 'SRV-012', category: '.netrc 파일 내 중요 정보 노출', dangerousLevel: '상', firstResult: '양호', lastResult: '취약', vulnerable: 'ckeck',  exception: '', state: '양호', record: 'check'},
